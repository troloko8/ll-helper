...
| Top-level request/response DTO records (excl. enums/nested) | 17 | 17 (Auth 3, User 3, Deck 3, Card 3, Learning 5) | ✅ |
...
| LEARN-02 | Learning | GET | `/api/v1/decks/{deckId}/study/cards` | `LearningController.getStudyCards` | JWT | Path: deckId | — | `List<DeckCardResponse>` | 200 | 409 not enrolled | implemented | Study screen | `LearningServiceImpl.getStudyCards`; `LearningServiceImplTest`; `LearningController.getStudyCards` | Selects max 10 in status priority `LEARNING` → `REVIEWING` → `NEW`, then `card.id ASC` inside each status; `MASTERED` is excluded (resolved Discrepancy K). |
...
- **Public/private Deck access:**
  ...
  - `GET /decks/{id}/study/cards` and `GET /decks/{id}/cards` (learning) remain implicitly gated by the enrollment requirement (§ below).
...
**Count: 7 confirmed candidate backend gaps** (main table below). Items that are frontend integration mappings, confirmed absent/out-of-current-scope capabilities, or known deferred limitations are listed separately in §9.1–§9.3 and are **not** included in this count. G-04/Discrepancies C and D, G-05/Discrepancies F and H, G-06, and G-08/Discrepancy K are resolved.
...
| Manual Card CRUD | partial | CARD-01,03,05,06 implemented; CARD-03 inherits parent-deck visibility; CARD-04 (list) remains unfiltered/missing deck-scoping |
...
- **26 existing HTTP operations:** 23 `implemented`, 3 `partial`, 0 `planned`, 0 `missing`, 0 `unclear` (§4). `DECK-03` is public-only, DECK-06 is owner-scoped, and `CARD-04` remains partial/unsafe.
...
- **Confirmed security/filtering gaps (1):** `GET /cards` remains unfiltered (§9); G-04 closed the `GET /decks` collection exposure and G-05 closed detail-read exposure. One additional error-contract gap — the catch-all 500 message leak — is documented in §9 but is not a privacy/filtering gap specifically. The JWT filter exception path (formerly Discrepancy O) is resolved — see §6/§7/§8.
...
Total candidate backend gaps counted in §9's main table: **7**. Endpoint-operation counts (26 total, 23 implemented / 3 partial) and domain-capability counts (§11) are tracked separately and must not be summed together.
