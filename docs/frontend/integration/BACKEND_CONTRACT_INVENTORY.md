...
| LEARN-02 | Learning | GET | `/api/v1/decks/{deckId}/study/cards` | `LearningController.getStudyCards` | JWT | Path: deckId | — | `List<DeckCardResponse>` | 200 | 409 not enrolled | partial | Study screen | `LearningServiceImpl.getStudyCards`; `LearningController.getStudyCards` | Selects only `LEARNING` then `NEW`, max 10 — `REVIEWING`/`MASTERED` excluded from study selection (Discrepancy K). |
...
**Count: 12 confirmed candidate backend gaps** (main table below). Items that are frontend integration mappings, confirmed absent/out-of-current-scope capabilities, or known deferred limitations are listed separately in §9.1–§9.3 and are **not** included in this count. G-06 is resolved by LEARN-05.
...
- **25 existing HTTP operations:** 18 `implemented`, 7 `partial`, 0 `planned`, 0 `missing`, 0 `unclear` (§4). `CARD-04` is `partial`, not `missing` — it is an existing, unsafe/unfiltered endpoint with a fully traced controller → service → repository path.
...
Total candidate backend gaps counted in §9's main table: **12**. Endpoint-operation counts (25 total, 18 implemented / 7 partial) and domain-capability counts (§11) are tracked separately and must not be summed together.
