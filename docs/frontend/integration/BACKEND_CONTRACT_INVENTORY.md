...
| DECK-03 | Deck | GET | `/api/v1/decks` | `DeckController.getPublicDecks` | JWT | — | — | `List<DeckListResponse>` (no cards) | 200 | — | implemented | Public deck collection | `DeckServiceImpl.getPublicDecks`, `DeckRepository.findAllByIsPublicTrue`, `DeckServiceImplTest`, `DeckControllerTest` | Repository-enforced `isPublic=true` filter prevents private-deck disclosure (G-04 / Discrepancy C resolved). No pagination, card count, enrollment state, or search parameters. |
| DECK-06 | Deck | GET | `/api/v1/decks/mine` | `DeckController.getCurrentUserDecks` | JWT | Current user from JWT | — | `List<DeckListResponse>` (no cards) | 200 | 404 current User absent | implemented | Created Decks | `DeckServiceImpl.getCurrentUserDecks`, `DeckRepository.findAllByOwnerId`, `DeckServiceImplTest`, `DeckControllerTest` | Returns every deck owned by the current user, including public and private decks. No pagination or card count. |
...
| C | `GET /api/v1/decks` returns every Deck via `findAll()`, no owner/public filtering | **resolved** | `DeckServiceImpl.getPublicDecks()` uses `DeckRepository.findAllByIsPublicTrue()` with an owner entity graph. Service and controller tests protect the public-only collection contract. | Private decks are no longer transmitted by the collection endpoint. G-04 is closed. |
| D | No separate Created Decks list endpoint | **resolved** | `GET /decks/mine` resolves the current user server-side and uses `DeckRepository.findAllByOwnerId(...)`. Service and controller tests cover both public and private owned decks. | Created Decks now has a distinct owner-scoped contract without weakening the public-only `GET /decks` boundary. |
| E | No safe Discover/public Deck list endpoint | **partially resolved** | `GET /decks` is now repository-filtered to `isPublic=true`, but has no search/query parameters and its response lacks the canonical Discover fields `cardCount` and `isEnrolled`. | The privacy blocker is resolved. Discover remains contract-incomplete until its required response fields are added; search/sort/pagination remain undefined or deferred. |
...
| Discover contract lacks required fields/query capabilities | incomplete response/query contract | DECK-03 is a safe public-only list, but still lacks `cardCount`, per-user `isEnrolled`, and defined Discover query behavior; see Discrepancy E. | `DeckController`, `DeckListResponse` |
...
| Public Decks (list) | implemented | DECK-03 returns only `isPublic=true`; richer Discover fields remain missing |
