...
> **Date:** 2026-08-30
> **Repository baseline:** G-06 Learning Decks implementation state documented on 2026-08-30.
...
|---|---|---|---|
| Controllers | 6 | 6 (`AuthController`, `UserController`, `DeckController`, `CardController`, `CardGenerationController`, `LearningController`) | ✅ |
| HTTP operations | 27 | 27 (Auth 2, User 7, Deck 6, Card 7, Learning 5) | ✅ |
| Top-level request/response DTO records (excl. enums/nested) | 19 | 19 (Auth 3, User 3, Deck 3, Card 4, Learning 6) | ✅ |
...
**Endpoint status summary (27 total):** `implemented` 25 · `partial` 2 · `planned` 0 · `missing` 0 · `unclear` 0. Every `Status` value in the table below sums to this total exactly. `missing` is reserved for product capabilities with no controller/route at all (see §9).
...
| USER-07 | User | GET | `/api/v1/users/me` | `UserController.getCurrentUser` | JWT | Authenticated principal email | — | `UserResponse` | 200 | 404 profile not found | implemented | Session bootstrap | `user/controller/UserController.java`, `user/service/UserServiceImpl.java`, `UserControllerTest.java`, `JwtSecurityFilterChainTest.java` | Resolves JWT subject email to `AuthUser`, then linked `User`; does not auto-create a profile. Shared missing/malformed/expired/invalid JWT contract is `401 {"message":"Authentication required"}`. |
...
| DECK-02 | Deck | GET | `/api/v1/decks/{id}` | `DeckController.getById` | JWT | Path: id | — | `DeckResponse` (with `cards`) | 200 | 403 private deck owned by another user, 404 | implemented | Owner/Public Deck Details | `DeckServiceImpl.getById`, `DeckAccessPolicy`, `DeckServiceImplTest`, `DeckControllerTest` | Public decks are readable by any authenticated user; private decks are readable only by their owner. G-05 / Discrepancy F resolved. |
| DECK-03 | Deck | GET | `/api/v1/decks` | `DeckController.getPublicDecks` | JWT | Current user from JWT | — | `List<PublicDeckListResponse>` (`id`, `title`, language pair, full `UserResponse owner`, `cardCount`, `isEnrolled`) | 200 | 404 current User absent | implemented | Public deck collection / Discover | `DeckServiceImpl.getPublicDecks`, `DeckRepository.findPublicDecks`, `DeckServiceImplTest`, `DeckControllerTest` | One aggregate native SQL query returns public decks with their owners, content-card counts including `0`, and ACTIVE enrollment state isolated to the current user. Private decks remain excluded. A future compact owner DTO is deferred; owner data itself is required. No pagination or search parameters. |
| DECK-06 | Deck | GET | `/api/v1/decks/mine` | `DeckController.getCurrentUserDecks` | JWT | Current user from JWT | — | `List<OwnedDeckListResponse>` (`id`, `title`, language pair, `isPublic`, `cardCount`) | 200 | 404 current User absent | implemented | Created Decks | `DeckServiceImpl.getCurrentUserDecks`, `DeckRepository.findOwnedDecks`, `DeckServiceImplTest`, `DeckControllerTest` | One aggregate native SQL query returns every deck owned by the current user with public/private visibility and content-card count including `0`. Owner is implied by the authenticated scope and is not selected or returned. No pagination. |
...
| CARD-01 | Card | POST | `/api/v1/decks/{deckId}/cards` | `CardController.create` | JWT | Path: deckId, Body | `CardRequest` | `CardResponse` | 201 | 400 validation, 403 not deck owner, 404 deck, 429 rate limit | implemented | Manual Add Card | `CardServiceImpl.create`, `CardControllerTest` | `deckId` is a positive path parameter. Body requires title/translation; definition is optional. Never calls AI. |
| CARD-02 | Card | POST | `/api/v1/card-generations/bulk` | `CardGenerationController.generateBulk` | JWT | Body | `BulkCardGenerateRequest` | `List<CardResponse>` (successes only) | 201 | 400 size limit, 403 not deck owner, 404 deck, 429 rate limit (endpoint + AI), 503 AI unavailable | partial | AI generate cards | `card/service/CardServiceImpl.java:115-157` | Failed titles are logged (`log.debug`/`log.warn`) but never returned to the client — Discrepancy N. |
| CARD-03 | Card | GET | `/api/v1/cards/{id}` | `CardController.getById` | JWT | Path: id | — | `CardResponse` | 200 | 403 private parent deck owned by another user, 404 card/deck | implemented | Add/Edit Card prefill | `CardServiceImpl.getById`, `DeckAccessPolicy`, `CardServiceImplTest`, `CardControllerTest` | Card visibility inherits the parent deck rule. G-05 / Discrepancy H resolved. |
| CARD-04 | Card | GET | `/api/v1/cards` | `CardController.getPublicCards` | JWT | — | — | `List<CardResponse>` | 200 | — | implemented | Public card collection | `CardServiceImpl.getPublicCards`, `CardRepository.findAllByDeckIsPublicTrue`, `CardServiceImplTest`, `CardControllerTest` | Repository-enforced parent `Deck.isPublic=true` filter prevents private-card disclosure (Discrepancy G resolved). No pagination. Owner inventory remains available through `DECK-02`; this endpoint is distinct from learning-scoped `LEARN-03`. |
| CARD-05 | Card | PUT | `/api/v1/cards/{id}` | `CardController.update` | JWT | Path: id, Body | `CardRequest` | `CardResponse` | 200 | 400 validation, 403 not deck owner, 404, 429 rate limit | implemented | Edit Card | `CardServiceImpl.update`, `CardControllerTest` | Same content body as manual create: title/translation required. Omitted optional fields are cleared; parent deck is unchanged. |
| CARD-07 | Card | POST | `/api/v1/card-generations` | `CardGenerationController.generate` | JWT | Body | `GenerateCardRequest` | `CardResponse` | 201 | 400 validation, 403 not owner, 404 deck, 429, 503 AI unavailable/invalid result | implemented | Single-card AI | `CardServiceImpl.generate`, `CardControllerTest` | Title and deckId only. Owner checked before AI; output validated and saved immediately. Shares CARD_CREATE per-user bucket with manual create. |
...
| LEARN-01 | Learning | POST | `/api/v1/decks/{deckId}/enroll` | `LearningController.enrollDeck` | JWT | Path: deckId | — | `EnrollResponse { userDeckId }` | 201 | 403 private deck, 404 deck, 409 already enrolled | implemented | Enroll action (from Deck Details Public) | `LearningServiceImpl.enrollDeck`; `LearningServiceImplTest` enroll scenarios; `LearningControllerTest` enroll scenarios | Duplicate-enroll caught via `DataIntegrityViolationException` message match on `uk_user_deck_progress_user_deck` — fragile (documented FIXME). |
| LEARN-02 | Learning | GET | `/api/v1/decks/{deckId}/study` | `LearningController.getStudySession` | JWT | Path: deckId | — | `StudySessionResponse {deckId, deckTitle, cards}` | 200 | 409 not enrolled; 404 deck missing | implemented | Study screen | `LearningServiceImpl.getStudySession`; `LearningServiceImplTest`; `LearningController.getStudySession` | Selects max 10 in status priority `LEARNING` → `REVIEWING` → `NEW`, then `card.id ASC` inside each status; `MASTERED` is excluded (resolved Discrepancy K). |
| LEARN-03 | Learning | GET | `/api/v1/decks/{deckId}/cards` | `LearningController.getDeckCards` | JWT | Path: deckId | — | `List<DeckCardResponse>` | 200 | 409 not enrolled | implemented | Learning Deck Details (card list with progress) | `LearningServiceImpl.getDeckCards`; `LearningServiceImpl.loadDeckCardsWithProgress`; `LearningController.getDeckCards` | Returns all enrolled cards with `CardProgressInfo`. Not an owner content-management list — see Discrepancy I. |
| LEARN-04 | Learning | POST | `/api/v1/cards/{cardId}/review` | `LearningController.reviewCard` | JWT | Path: cardId, Body | `CardReviewRequest` | `CardReviewResponse` | 200 | 400 validation, 409 not enrolled, 404 card/progress | implemented | Study answer submission | `LearningServiceImpl.reviewCard`; `LearningServiceImplTest` review/status-transition scenarios; `LearningControllerTest` review scenarios | Answer check: `userAnswer.trim().equalsIgnoreCase(card.title.trim())`. Correctness always backend-computed — matches `DESIGN.md` domain boundary. |
| LEARN-05 | Learning | GET | `/api/v1/learning/decks` | `LearningController.getMyDecks` | JWT | — | — | `List<LearningDeckResponse>` | 200 | — | implemented | Learning dashboard | `learning/service/LearningServiceImpl.getMyDecks`, `LearningServiceImplTest`, `LearningControllerTest` | Returns only current-user `ACTIVE` enrollments. Studied decks sort first by `lastStudiedAt DESC`; never-studied decks follow by `enrolledAt DESC`; `id ASC` is the deterministic tie-breaker. Progress is aggregated in batch as mastered/total counts. |

No dedicated "Discover"/public search or aggregate cross-deck "Progress" endpoint exists in the controllers inspected (see §5, §9). The owner-scoped Created Decks collection (DECK-06), `GET /api/v1/users/me` (USER-07), and the Learning Decks list (LEARN-05) are implemented.
...
| `PublicDeckListResponse` | response | id, title, sourceLanguage, targetLanguage, owner, cardCount, isEnrolled | same | mixed | all fields non-null; `cardCount` may be `0` | Discover-specific shape. `owner` is the full nested `UserResponse`; compacting that representation is deferred. `isEnrolled=true` only for the current user's ACTIVE enrollment; ownership is independent. | nested `UserResponse owner` | `deck/dto/response/PublicDeckListResponse.java`, `DeckRepository.PublicDeckListProjection` |
| `OwnedDeckListResponse` | response | id, title, sourceLanguage, targetLanguage, isPublic, cardCount | same | mixed | all fields non-null; `cardCount` may be `0` | Minimal Created-specific shape; owner is implied by the authenticated owner-scoped endpoint | none | `deck/dto/response/OwnedDeckListResponse.java`, `DeckRepository.OwnedDeckListProjection` |
| `DeckResponse` | response | id, title, sourceLanguage, targetLanguage, createdAt, updatedAt, owner, isPublic | same | mixed | JSON property always present; value non-null | Mirrors non-null entity fields and DB-managed timestamps | nested `UserResponse owner` | `deck/dto/response/DeckResponse.java` |
| `DeckResponse` | response | description | `description` | `String` | JSON property always present; value may be `null` | `Deck.description` is nullable | — | same |
...
**List responses vs `DeckResponse`:** Discover includes the current full `UserResponse owner`; Created omits owner because the endpoint is already scoped to the authenticated owner. `GET/POST/PUT /decks/{id}` use `DeckResponse`, which additionally carries description, timestamps and cards.
...
| `CardRequest` | request | definition | `definition` | `String` | optional | `@Size(max=1000)` | — | same |
...
| `CardRequest` | request | translation | `translation` | `String` | required | `@NotBlank(message="Translation is required") @Size(max=200)` | — | same |
| `GenerateCardRequest` | request | title, deckId | same | `String`, `Long` | required | title: `@NotBlank @Size(max=100)`; deckId: `@NotNull @Positive` | — | `card/dto/request/GenerateCardRequest.java` |
...
| `CardResponse` | response | definition, synonyms, examples, translation | same | `String`/`List<String>` | Existing legacy rows may contain nulls; every new manual or AI card and every PUT requires translation | Blank definition is normalized to null. AI output without a non-blank translation is rejected before persistence. No database nullability change. | — | `CardServiceImpl`, request records |
...
| `LearningDeckResponse` | response | deckId, title, sourceLanguage, targetLanguage | same | `Long`/`String`/`Language` | JSON property always present; value non-null | Joined from the active enrollment's `Deck` | enum `Language` | `learning/dto/response/LearningDeckResponse.java`, `LearningMapper.toLearningDeckResponse` |
| `LearningDeckResponse` | response | enrolledAt, lastStudiedAt | same | `Instant` | `enrolledAt` non-null; `lastStudiedAt` nullable | `enrolledAt` is application-managed on enrollment and backfilled by V11; `lastStudiedAt` remains null until the first review | — | `learning/entity/UserDeckProgress.java`, V11 migration |
| `LearningDeckResponse` | response | progress | `progress` | `ProgressSummary` | always present; value non-null | Batch-aggregated from all `UserCardProgress` rows for the enrollment | nested `{masteredCount: long, totalCount: long}` | `LearningServiceImpl.getMyDecks`, `LearningDeckResponse.ProgressSummary` |
| `StudySessionResponse` | response | deckId, deckTitle, cards | same | `Long`/`String`/`List<DeckCardResponse>` | Always present, non-null; cards may be empty | Enrolled deck metadata and ordered study batch (max 10) | No Learning-list request required for Study title | `learning/dto/response/StudySessionResponse.java` |
...
`common/model/Language.java` — ISO 639-1 codes; used by `Deck.sourceLanguage`/`targetLanguage` (JPA `@Enumerated(STRING)`) and deck request/detail/list DTOs. Not used for `User.nativeLanguage`/`targetLanguage`/`uiLanguage`, which are plain validated strings (pattern-only, no enum).
...
- **401, case 2 — malformed/expired/invalid-signature JWT (confirmed controlled 401):** `JwtAuthenticationFilter.doFilterInternal` (`common/security/JwtAuthenticationFilter.java`) wraps `jwtService.extractUsername(jwt)`/`isTokenValid(...)` in a `try`/`catch (JwtException | IllegalArgumentException e)`. On catch, it clears `SecurityContextHolder`, delegates to the shared `RestAuthenticationEntryPoint.commence(...)` (the same bean used by `SecurityConfig`'s `authenticationEntryPoint`, so the JSON-writing logic is not duplicated), and returns without calling `filterChain.doFilter` — no double response write. This produces the identical `{"message":"Authentication required"}` 401 body as case 1, for expired (`ExpiredJwtException`), malformed (`MalformedJwtException`), and invalid-signature (`SignatureException`) tokens — all subtypes of `io.jsonwebtoken.JwtException`. Confirmed by a dedicated filter-chain test that runs the real `SecurityFilterChain` (`JwtSecurityFilterChainTest`, `common/security/JwtSecurityFilterChainTest.java`) — unlike the controller slice tests, which use `@AutoConfigureMockMvc(addFilters=false)` and do not exercise this path. Previously tracked as Discrepancy O (§8) and an unresolved question (§10); now resolved.
...
- **Public/private Deck access:**
  ...
  - `GET /decks/{id}` — `DeckAccessPolicy` permits public decks or the current owner's private deck; another user's private deck returns `403 {"message":"Access denied: private deck"}`.
  - `GET /cards/{id}` — resolves the parent deck and applies the same `DeckAccessPolicy`; card visibility cannot diverge from deck visibility.
  - `GET /decks/{id}/study` and `GET /decks/{id}/cards` (learning) remain implicitly gated by the enrollment requirement (§ below).
- **Enrollment requirement:** `getStudySession`/`getDeckCards`/`reviewCard` all require an existing `UserDeckProgress` for `(userId, deckId)`; if absent, `LearningServiceImpl.loadDeckCardsWithProgress`/`reviewCard` throw `IllegalStateException("Deck not enrolled. Please enroll first.")`, which `GlobalExceptionHandler.handleIllegalState` maps to **`409 Conflict`** — not `403`. `docs/features/learning-flow.md` §5.2 was corrected to match actual `409` behavior (G-12, resolved).
...
- **Current-user session bootstrap:** `GET /api/v1/users/me` resolves the authenticated JWT subject email to `AuthUser`, then resolves the linked `User` and returns `UserResponse`. A valid JWT without a profile returns `404 {message}` and does not create one; missing/malformed/expired/invalid JWT returns the shared controlled `401 {"message":"Authentication required"}`.
...
|---|---|---|---|---|---|
| 400 | `{"errors": {field: message}}` | Bean validation failure (`@Valid` body or constrained controller parameter) | Endpoints with validated request bodies/path parameters | `GlobalExceptionHandler.handleMethodArgumentNotValid` / `handleHandlerMethodValidation` | Existing validation tests plus `CardControllerTest.create_shouldReturn400_whenDeckIdIsNotPositive` and translation validation on nested card creation |
...
| 400 | `{"message": "..."}` | `IllegalArgumentException` (e.g. bulk size exceeds configured `ai.max-bulk-size`) | `POST /card-generations/bulk` | `GlobalExceptionHandler.handleIllegalArgument`, `CardServiceImpl.validateBulkSize` | Service-level `IllegalArgumentException` behavior **is** directly tested: `CardServiceImplTest.generateBulk_shouldThrowBadRequest_whenSizeExceedsLimit` (above the configured limit throws) and `generateBulk_shouldNotThrow_whenSizeEqualsLimit` (at-limit boundary does not throw). `GlobalExceptionHandler.handleIllegalArgument`'s `IllegalArgumentException → 400` mapping is proven by code inspection only (`GlobalExceptionHandler.java:73-77`), not a dedicated test. No `@WebMvcTest`/controller-level test exercises the complete HTTP path for this specific `validateBulkSize` scenario — `CardControllerTest.generateBulk_shouldReturn400_whenSizeExceedsLimit` exercises a different validation layer (the DTO-level `@Size(max=100)` on `titles`). |
| 401 | `{"message":"Authentication required"}` | Missing Bearer token (no `Authorization` header, or no `Bearer ` prefix) — `JwtAuthenticationFilter` passes through unauthenticated, request reaches `FilterSecurityInterceptor` unauthenticated | Any secured endpoint (shared contract for every `Auth = JWT` row in §4 — see §6) | `SecurityConfig` custom `authenticationEntryPoint` (`RestAuthenticationEntryPoint`) | Confirmed by code inspection (this path never throws) and by `JwtSecurityFilterChainTest.getUserById_shouldReturn401_whenNoAuthorizationHeader`, which runs the real filter chain |
| 401 | `{"message":"Authentication required"}` | Malformed, expired, or invalid-signature Bearer token — `JwtAuthenticationFilter` catches `JwtException`/`IllegalArgumentException` from `JwtService`, clears `SecurityContextHolder`, and delegates to the same `RestAuthenticationEntryPoint` used for the missing-token case | Any secured endpoint with an invalid (not missing) JWT | `JwtAuthenticationFilter` catch block → `RestAuthenticationEntryPoint.commence(...)` | Confirmed by `JwtSecurityFilterChainTest.getUserById_shouldReturn401_whenBearerTokenIsMalformed` / `...whenJwtIsExpired` / `...whenJwtHasInvalidSignature` — all run the real `SecurityFilterChain`. Previously Discrepancy O (§8), now resolved |
...
| 403 | `{"message": "..."}` | `AccessDeniedException` — ownership/visibility violation | Deck read/update/delete, Card read/create/update/delete/bulk-generate, User update/delete, Deck enroll (private deck) | `GlobalExceptionHandler.handleAccessDenied` | `DeckControllerTest.getById_shouldReturn403_whenPrivateDeckIsOwnedByAnotherUser` / `update_shouldReturn403_whenNotOwner`, `CardControllerTest.getById_shouldReturn403_whenPrivateParentDeckIsOwnedByAnotherUser` / `create_shouldReturn403_whenNotDeckOwner`, `UserControllerTest.update_shouldReturn403_whenNotSelf`, service-level visibility/ownership tests |
...
| 503 | `{"message": "AI service unavailable: ..."}` | `AiServiceException` — API key missing, OpenAI HTTP error, JSON parse failure | Single-card generation, bulk generation | `GlobalExceptionHandler.handleAiServiceException` | `CardControllerTest.generate_shouldReturn503_whenProviderFails`; provider unit tests cover the exception itself |
...
| C | `GET /api/v1/decks` returns every Deck via `findAll()`, no owner/public filtering | **resolved** | `DeckServiceImpl.getPublicDecks()` uses the public-filtered aggregate native query `DeckRepository.findPublicDecks(...)`. Service and controller tests protect the public-only collection contract. | Private decks are no longer transmitted by the collection endpoint. G-04 is closed. |
| D | No separate Created Decks list endpoint | **resolved** | `GET /decks/mine` resolves the current user server-side and uses `DeckRepository.findOwnedDecks(...)`. Service and controller tests cover both public and private owned decks. | Created Decks has a distinct owner-scoped contract without weakening the public-only `GET /decks` boundary. |
| E | No safe Discover/public Deck list endpoint | **resolved for the accepted bounded list** | `GET /decks` is repository-filtered to `isPublic=true` and includes aggregate `cardCount` plus current-user ACTIVE `isEnrolled`. It intentionally has no search/sort/pagination parameters. | The accepted Discover list contract is complete. Search, sort and pagination remain deferred capabilities rather than gaps in the bounded list. |
| F | `DeckServiceImpl.getById()` did not enforce private-deck visibility | **resolved** | `DeckServiceImpl.getById()` now loads the owner and applies shared `DeckAccessPolicy`: public or owner-private reads succeed; another user's private deck returns 403. Covered by service and controller tests. | Owner/Public Deck Details now have a backend-enforced read trust boundary; frontend guards remain UX only, not authorization. |
| G | `GET /api/v1/cards` returned every Card globally | **resolved** | `CardServiceImpl.getPublicCards()` uses `CardRepository.findAllByDeckIsPublicTrue()`. Service and controller tests protect the public-parent-deck-only collection contract. | Private-deck cards are no longer transmitted by the global collection endpoint. Owner inventory continues to use the cards nested in `DECK-02`. |
| H | `CardServiceImpl.getById()` did not enforce ownership/visibility | **resolved** | `CardServiceImpl.getById()` now loads the parent deck and applies the shared `DeckAccessPolicy`. Covered by service and controller tests. | Card prefill can expose public-deck cards or the owner's private cards, but rejects another user's private content with 403. |
...
| J | No Learning Decks list or aggregate Progress endpoint | **partially resolved** | G-06 added `GET /api/v1/learning/decks`, backed by active current-user enrollments and batch progress aggregation. A separate cross-deck Progress dashboard contract still does not exist. | The Learning dashboard now has an implemented backend contract. The deferred `/progress` surface remains without an aggregate contract (see §9). |
| K | `getStudySession()` previously excluded REVIEWING | **resolved** | `LearningServiceImpl.getStudySession()` now ranks eligible cards `LEARNING` → `REVIEWING` → `NEW`, sorts by `card.id ASC` inside each status, limits the result to 10, and excludes only `MASTERED`. Covered by service unit tests. | `REVIEWING` cards can re-surface in Study and progress to `MASTERED`; an empty response no longer hides pending `REVIEWING` cards. |
...
| M | Creator Profile has User lookup endpoints but no confirmed creator-owned/public Decks endpoint | **confirmed** | `UserController` provides profile lookup only. `GET /decks` is now public-only but accepts no creator/owner filter. | Private data is no longer exposed, but Creator Profile still lacks a creator-scoped server contract and per-deck card count. |
...
| O | Invalid/expired JWT exceptions may occur inside the filter before MVC exception handling | **resolved — controlled 401** | `JwtAuthenticationFilter.doFilterInternal()` now wraps `jwtService.extractUsername(jwt)`/`isTokenValid(...)` in `try/catch (JwtException | IllegalArgumentException e)`, clears `SecurityContextHolder`, and delegates to the shared `RestAuthenticationEntryPoint` bean (the same one registered as `SecurityConfig`'s `authenticationEntryPoint`), returning before `filterChain.doFilter`. Confirmed by `JwtSecurityFilterChainTest` (`common/security/JwtSecurityFilterChainTest.java`), a dedicated test that runs the real `SecurityFilterChain` (not `addFilters=false`), covering missing header, malformed token, expired token, invalid signature, and the valid-JWT pass-through case. | Malformed/expired/invalid-signature Bearer tokens now return the same `{"message":"Authentication required"}` 401 body as the missing-token case (§6/§7) — frontend can rely on a single generic 401 handler for all invalid-auth cases without distinguishing token failure modes. |
...
**Count: 4 confirmed candidate backend gaps** (main table below). Items that are frontend integration mappings, confirmed absent/out-of-current-scope capabilities, or known deferred limitations are listed separately in §9.1–§9.3 and are **not** included in this count. G-04/Discrepancies C and D, G-05/Discrepancies F and H, G-06, G-08/Discrepancy K, CARD-04/Discrepancy G, the Discover collection contract, and the catch-all 500 message leak are resolved.
...
|---|---|---|
| `isPublic` (backend) ↔ `isPrivate` (UI) | `DeckRequest`/`OwnedDeckListResponse`/`DeckResponse` use `isPublic`; `docs/frontend/DESIGN.md` describes a "Private Deck / `isPrivate`" UI control. This is a **required frontend-side boundary mapping** (`isPrivate = !isPublic`) at the form/DTO boundary — the backend field itself is complete and functional. | Discrepancy L; `deck/dto/request/DeckRequest.java:17` |
...
**Count: 0 unresolved questions.** The Created Decks collection is resolved by DECK-06 `GET /api/v1/decks/mine`; the previously resolved JWT, Register→Profile, and private detail-read decisions remain unchanged.
...
| User profile | implemented | USER-01..07 |
| Created/owned Decks (list) | implemented | DECK-06 returns the current user's public and private decks; DECK-03 remains public-only |
| Public Decks (list) | implemented | DECK-03 returns only `isPublic=true` with full owner data, `cardCount` and current-user ACTIVE `isEnrolled` |
| Deck CRUD | implemented | DECK-01,02,04,05 implemented; DECK-02 enforces public-or-owner read visibility |
| Manual Card CRUD | implemented | CARD-01,03,05,06 implemented; CARD-03 inherits parent-deck visibility; CARD-04 is a safe public-parent-deck-only collection; owner inventory uses DECK-02 |
| AI generation | partial | CARD-07 (single), CARD-02 (bulk) implemented; CARD-02 drops failed-title reporting |
| Discover | implemented for the accepted bounded list | DECK-03 provides the public collection fields required by Discover; search, sort and pagination are deferred |
...
| Learning Decks (list) | implemented | LEARN-05 (`GET /api/v1/learning/decks`) |
| Learning Deck Details | implemented | LEARN-03 (`DeckCardResponse` with progress) |
| Study | implemented | LEARN-02 includes `LEARNING`, `REVIEWING`, and `NEW` in the accepted priority order; `MASTERED` is excluded |
