...
| HTTP operations | 25 | 25 (Auth 2, User 7, Deck 5, Card 6, Learning 5) | ✅ |
...
No separate "Created Decks" list, "Discover"/public search, or aggregate cross-deck "Progress" endpoint exists in the controllers inspected (see §5, §9). `GET /api/v1/users/me` (USER-07) and the Learning Decks list (LEARN-05) are implemented.
...
**Count: 9 confirmed candidate backend gaps** (main table below). Items that are frontend integration mappings, confirmed absent/out-of-current-scope capabilities, or known deferred limitations are listed separately in §9.1–§9.3 and are **not** included in this count. G-05/Discrepancies F and H, G-06, and G-08/Discrepancy K are resolved.
...
**Count: 1 unresolved question.** (Previously 4 — expired/malformed JWT resolved §6/§7/§8; Register→Profile UX resolved by Phase 0.4C accepted flow (`POST /users` on `/onboarding/profile`); private-deck exposure accepted as a temporary Level-1 gap in `current-architecture.md` §16.)
...
- "Created" (owner content management) and "Learning" (enrolled study) remain distinct product collections. Created/Discover surfaces cannot be safely derived by client-side filtering of the unfiltered `GET /decks` (see Discrepancy C) and are **blocked** pending backend fixes; Learning has its own current-user-scoped LEARN-05 contract.

## 12. Phase 0.4A summary

- **25 existing HTTP operations:** 21 `implemented`, 4 `partial`, 0 `planned`, 0 `missing`, 0 `unclear` (§4). `CARD-04` is `partial`, not `missing` — it is an existing, unsafe/unfiltered endpoint with a fully traced controller → service → repository path.
- **Missing product capabilities/backend contracts (4):** Created Decks list, Discover/public Deck list, aggregate Progress endpoint, Creator-owned/public Decks endpoint (§9).
- **Confirmed security/filtering gaps (2):** `GET /decks` unfiltered and `GET /cards` unfiltered (§9). G-05 closed the detail-read exposure for `GET /decks/{id}` and `GET /cards/{id}`. One additional error-contract gap — the catch-all 500 message leak — is documented in §9 but is not a privacy/filtering gap specifically. The JWT filter exception path (formerly Discrepancy O) is resolved — see §6/§7/§8.
...
- **Unresolved questions (1):** `GET /decks` split-vs-filter backend approach (§10). Register→Profile UX sequence and private detail-read visibility are resolved.

Total candidate backend gaps counted in §9's main table: **9**. Endpoint-operation counts (25 total, 21 implemented / 4 partial) and domain-capability counts (§11) are tracked separately and must not be summed together.
