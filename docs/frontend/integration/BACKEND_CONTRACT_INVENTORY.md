...
| HTTP operations | 24 | 24 (Auth 2, User 7, Deck 5, Card 6, Learning 4) | ✅ |
...
No separate "Created Decks" list, "Discover"/public search, aggregate "Progress"/"Learning Decks" list exists in the controllers inspected (see §5, §9). `GET /api/v1/users/me` is implemented (USER-07).
...
**Count: 13 confirmed candidate backend gaps** (main table below). Items that are frontend integration mappings, confirmed absent/out-of-current-scope capabilities, or known deferred limitations are listed separately in §9.1–§9.3 and are **not** included in this count. (Previously 16 — JWT filter exception path, current-user endpoint gap, and G-12 documentation mismatch are resolved; see §6/§7/§8.)
...
- **24 existing HTTP operations:** 17 `implemented`, 7 `partial`, 0 `planned`, 0 `missing`, 0 `unclear` (§4). `CARD-04` is `partial`, not `missing` — it is an existing, unsafe/unfiltered endpoint with a fully traced controller → service → repository path.
- **Missing product capabilities/backend contracts (5):** Created Decks list, Discover/public Deck list, Learning Decks list, aggregate Progress endpoint, Creator-owned/public Decks endpoint (§9).
...
Total candidate backend gaps counted in §9's main table: **13** ("Not enrolled" 409 vs 403 mismatch removed — corrected in `learning-flow.md` as G-12). Endpoint-operation counts (24 total, 17 implemented / 7 partial) and domain-capability counts (§11) are tracked separately and must not be summed together.
