...
import { sessionAuthenticated, sessionCleared, sessionReducer } from './session-slice'
...
describe('session selectors', () => {
  it('selectSessionStatus returns the current status', () => {
    expect(selectSessionStatus({ session: { status: 'authenticated' } })).toBe('authenticated')
  })

  it('selectIsAuthenticated is true only when authenticated', () => {
    expect(selectIsAuthenticated({ session: { status: 'authenticated' } })).toBe(true)
    expect(selectIsAuthenticated({ session: { status: 'anonymous' } })).toBe(false)
    expect(selectIsAuthenticated({ session: { status: 'initializing' } })).toBe(false)
  })
})
